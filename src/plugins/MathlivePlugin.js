...
import { openMathliveModal } from './openMathliveModal';

// Inline SVG icon (replaces the math-min.png from CKEditor 4)
const MATH_ICON = `<?xml version="1.0" encoding="utf-8"?>
...
export default class MathlivePlugin extends Plugin {
  ...
  init() {
    ...
    // ── 1. Register schema: allow mathInline attribute on inline text nodes ──
    editor.model.schema.extend('$text', {
      allowAttributes: ['mathInline'],
    });

    // Allow mathInline on any element that allows $text
    editor.model.schema.setAttributeProperties('mathInline', {
      isFormatting: false,
      copyOnEnter: false,
    });

    // ── 2. Upcast: <span class="math-inline" data-latex="..."> → model ──
    editor.conversion.for('upcast').elementToAttribute({
      ...
      model: {
        key: 'mathInline',
        value: (viewElement) => viewElement.getAttribute('data-latex') || '',
      },
      converterPriority: 'high',
    });

    // ── 3. Downcast: model mathInline attr → <span class="math-inline" ...> ──
    editor.conversion.for('downcast').attributeToElement({
      model: 'mathInline',
      view: (latexValue, { writer }) => {
        if (!latexValue) return;
        const latex = String(latexValue);
        const encoded = latex.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        const span = writer.createAttributeElement(
          'span',
          {
            class: 'math-inline',
            'data-latex': latex,
    ...
    editor.ui.componentFactory.add('mathliveButton', (locale) => {
      ...
      button.on('execute', () => {
        ...
        // If an image is selected and it has alt text, try to edit it
        if (selectedElement && selectedElement.is('element', 'imageInline')) {
          const latex = selectedElement.getAttribute('alt');
          if (latex) {
            const viewElement = editor.editing.mapper.toViewElement(selectedElement);
            if (viewElement) {
              const wrapperElem = editor.editing.view.domConverter.mapViewToDom(viewElement);
              const domElement = wrapperElem.tagName === 'IMG' ? wrapperElem : wrapperElem.querySelector('img');
              if (domElement) {
                openMathliveModal(editor, { domElement, modelElement: selectedElement, latex });
                return;
              }
            }
          }
        ...
        // Default: Open in Insert Mode
        openMathliveModal(editor, null);
      });

      return button;
    });

    // ── 5. Double-click on existing math-inline span → open editor ──
    editor.editing.view.document.on(
      'click',
      (evt, data) => {
        if (data.domEvent.detail !== 2) return; // only double-clicks

        // Walk up the DOM target to find a math-inline span
        let el = data.domTarget;
        while (el && el !== document.body) {
          if (
            (el.tagName === 'SPAN' && el.classList && el.classList.contains('math-inline')) ||
            (el.tagName === 'IMG' && el.getAttribute('src') && el.getAttribute('src').startsWith('data:image'))
          ) {
            data.domEvent.preventDefault();
            evt.stop();

            const latex = el.getAttribute('data-latex') || el.getAttribute('alt') || '';
            const viewElement = editor.editing.view.domConverter.mapDomToView(el);
            
            let modelElement = null;
            let modelRange = null;

            if (viewElement) {
              if (viewElement.name === 'img' || viewElement.hasClass('image') || viewElement.hasClass('image-inline') || viewElement.is('element', 'imageInline')) {
                // It's an image. Try to map it directly or its parent (image widget wrapper)
                modelElement = editor.editing.mapper.toModelElement(viewElement) || editor.editing.mapper.toModelElement(viewElement.parent);
              } else if (viewElement.is('attributeElement') || viewElement.is('element', 'span')) {
                // For spans, the span is an attribute wrapped around text. We need the range.
                const viewRange = editor.editing.view.createRangeIn(viewElement);
                modelRange = editor.editing.mapper.toModelRange(viewRange);
              }
            }

            // Fallback for image: if mapper fails, use the current selection if it's an image
            if (!modelElement && el.tagName === 'IMG') {
              const selected = editor.model.document.selection.getSelectedElement();
              if (selected && (selected.is('element', 'imageInline') || selected.is('element', 'imageBlock'))) {
                modelElement = selected;
              }
            }

            openMathliveModal(editor, { domElement: el, modelElement, modelRange, latex });
            return;
          }
          el = el.parentElement;
        }
